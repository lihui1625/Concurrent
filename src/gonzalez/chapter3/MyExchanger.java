...
public class MyExchanger {
	...
	public static void main(String[] args) {
		...
		List<String> productBuffer = new ArrayList<String>();
		List<String> productBuffer2 = new ArrayList<String>();
		List<String> consumeBuffer = new ArrayList<String>();
		...
		Producer producer2 = new Producer(productBuffer2, exchanger);
		Consumer consumer = new Consumer(consumeBuffer, exchanger);
...
class Producer implements Runnable {
	...
	private List<String> productBuffer;
	...
	public void run() {
		...
		for (int i = 0; i < 10; i++) {
			...
			for (int j = 0; j < 10; j++) {
				...
				productBuffer.add(message);
			}

			// 7. 调用 exchange() 方法来与consumer交换数据。此方法可能会抛出InterruptedException
			// 异常, 加上处理代码。
			try {
				productBuffer = exchanger.exchange(productBuffer);
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			System.out.println("Producer: " + productBuffer.size());
			cycle++;
		}
	}
}
...
class Consumer implements Runnable {
	...
	public Consumer(List<String> consumeBuffer, Exchanger<List<String>> exchanger) {
	...
	public void run() {
		...
		for (int i = 0; i < 10; i++) {
			...
			// 加上处理代码。
			try {
				consumeBuffer = exchanger.exchange(consumeBuffer);
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			...
			// + buffer.size());
			for (int j = 0; j < 10; j++) {
				String message = consumeBuffer.get(0);
				System.out.println("Consumer: " + message);
				consumeBuffer.remove(0);
			}
			cycle++;
		}
	}
