...
public class MyCyclicBarrier {
	...
	public static void main(String[] args) {
		...
		MyCyclicBarrier mock = new MyCyclicBarrier(ROWS, NUMBERS, SEARCH);
	...
	public MyCyclicBarrier(int size, int length, int number) {
...
// 13. 现在你有了辅助类，是时候来实现线程了。首先，实现 Searcher
// 类。这个类会在随机数字的矩阵中的特定的行里查找数字。创建一个类名为Searcher 并一定实现 Runnable 接口.
class Searcher implements Runnable {
	...
	public Searcher(int firstRow, int lastRow, MyCyclicBarrier mock,
...
// 25. 现在，实现一个类来计算数字在这个矩阵里出现的总数。它使用储存了矩阵中每行里数字出现次数的 Results 对象来进行运算。创建一个类，名为
// Grouper 并一定实现 Runnable 接口.
